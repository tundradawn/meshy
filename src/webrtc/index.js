...
class WebRTC {
  ...
  connectPeer(clientId, sendAnswer) {
    ...
    if (!peer) {
      this._peers[clientId] = new PeerConnection(clientId, this._socket, this._config, callbacks);
  ...
  createDataChannels(clientId, labels = []) {
    const peer = this.getPeerById(clientId);

    if (peer) {
      peer.createDataChannels(labels);
    }
  }
